...
def build_recap(hist: pd.DataFrame,
    ...
    """One row per LB — identity + model choice + next-year prediction + confidence.
    No wide year columns → clean dimension table for Power BI."""
...
def build_donnees(hist: pd.DataFrame,
    ...
    for key in keys:
        ...
        # Historical years (not validation)
        hist_only_years = [y for y in grp.index if y not in val_years]
...
def main():
    ...
    val_years = all_years[2:]   # toutes les années sauf les 2 premières
    ...
    recap_df   = build_recap(hist, val_dfs, prev_df)
    donnees_df = build_donnees(hist, val_dfs, prev_df)
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        donnees_out["Année"] = donnees_out["Année"].astype(str)  # text for Power BI
