...
\begin{itemize}
    ...
    \item \textbf{La gestion des archives d'ex\'ecution financi\`ere}~: extraction, compilation et archivage des \'etats d'ex\'ecution budg\'etaire annuels produits par le syst\`eme GID -- mat\'erialis\'es notamment par les fichiers \textit{SituationChap-AAAA.xls} qui constituent la mati\`ere premi\`ere de notre analyse empirique (voir chapitre~4).
...
Dans l'exercice de ses missions, la Direction du Budget interagit avec plusieurs acteurs ext\'erieurs au minist\`ere. La \textbf{Direction du Budget du MEF} fixe les enveloppes dans les lettres de cadrage et arbitre les demandes lors des conf\'erences budg\'etaires annuelles. La \textbf{TGR} assure l'ex\'ecution comptable et le contr\^ole des d\'epenses via le syst\`eme de Gestion Int\'egr\'ee de la D\'epense (GID), d\'eploy\'e depuis 2009 pour offrir une vision en temps r\'eel de l'\'etat de consommation des cr\'edits. L'\textbf{Inspection G\'en\'erale des Finances (IGF)} proc\`ede \`a des audits p\'eriodiques de la gestion financi\`ere. La \textbf{Cour des Comptes} exerce le contr\^ole juridictionnel \textit{a posteriori} sur les comptes publics, et ses observations alimentent annuellement le dialogue de gestion entre le minist\`ere et le MEF.

C'est dans ce contexte que s'inscrit la probl\'ematique de ce m\'emoire. La Direction du Budget dispose d'historiques d'ex\'ecution dont la profondeur exploitable reste limit\'ee -- cinq exercices clos au moment du travail -- alors m\^eme que les m\'ethodes de pr\'evision usuelles et les outils r\'ecents de \textit{Machine Learning} sont calibr\'es pour des r\'egimes de donn\'ees beaucoup plus riches. \`A cela s'ajoutent des contraintes informatiques fortes au sein du minist\`ere, qui interdisent l'ex\'ecution d'applications compil\'ees non valid\'ees. Ceci soul\`eve la question centrale de notre recherche~:
...
Cette question ne pose pas la question na\"ive de la performance des mod\`eles riches en param\`etres~: elle interroge la \textbf{valeur ajout\'ee op\'erationnelle} d'un dispositif adapt\'e au terrain (d\'etection pr\'ecoce des lignes \`a risque, pr\'evisions ligne par ligne, aide \`a la r\'eallocation des cr\'edits) face \`a une simple extrapolation tendancielle ou \`a une projection passive bas\'ee sur l'ann\'ee pr\'ec\'edente.
...
Pour r\'epondre \`a ces enjeux, ce m\'emoire poursuit quatre objectifs articul\'es~:

\begin{itemize}
    \item \textbf{Constituer et structurer une base de donn\'ees d'ex\'ecution budg\'etaire~:}
    Extraire, nettoyer et consolider les fichiers \textit{SituationChap-AAAA.xls} produits annuellement par le minist\`ere -- en y appliquant le calcul du taux d'engagement annuel et l'identification des lignes stables sur l'ensemble de la p\'eriode \'etudi\'ee.

    \item \textbf{Justifier th\'eoriquement le choix d'une famille de mod\`eles parcimonieux~:}
    \`A partir du compromis biais-variance et des propri\'et\'es des estimateurs en r\'egime de donn\'ees rares, expliciter pourquoi les approches plus riches en param\`etres sont peu adapt\'ees au contexte \'etudi\'e, et pourquoi des mod\`eles simples (tendance lin\'eaire, moyennes, persistance, lissage exponentiel) constituent un choix m\'ethodologiquement coh\'erent plut\^ot qu'un repli par d\'efaut.

    \item \textbf{D\'evelopper un dispositif int\'egr\'e de pr\'evision et de d\'etection d'anomalies~:}
    \'Evaluer cinq mod\`eles parcimonieux avec s\'election ligne par ligne sur l'ann\'ee de validation (MAPE), produire une pr\'evision triennale assortie d'intervalles de confiance, et compl\'eter le dispositif par un module de d\'etection d'anomalies combinant un score~$Z$ sur le taux d'engagement annuel et un algorithme \textit{Isolation Forest} sur les caract\'eristiques structurelles des lignes. Les deux signaux sont crois\'es pour produire des niveaux de priorit\'e op\'erationnelle directement exploitables par le gestionnaire.

    \item \textbf{Concevoir et impl\'ementer un double livrable op\'erationnel~:}
    D\'evelopper d'une part un outil HTML autonome ex\'ecut\'e dans le navigateur du gestionnaire, qui ing\`ere les fichiers \textit{.xls} bruts et g\'en\`ere localement les rapports finaux (pr\'evisions et anomalies) sans installation ni transfert de donn\'ees~; et d'autre part un tableau de bord interactif Power BI qui consomme ces sorties et offre une vue de pilotage \`a la Direction du Budget. Ce double livrable r\'epond aux contraintes de s\'ecurit\'e informatique du minist\`ere, qui interdisent l'ex\'ecution d'applications compil\'ees non valid\'ees.
\end{itemize}
...
La Division de la pr\'evision et de la programmation financi\`ere produit des pr\'evisions budg\'etaires annuelles \`a partir de m\'ethodes qui restent, \`a ce jour, insuffisamment outill\'ees. C'est pr\'ecis\'ement cette lacune que ce m\'emoire se propose de combler, en construisant un dispositif quantitatif explicitement con\c{c}u pour le r\'egime de donn\'ees rares qui caract\'erise le contexte~: un syst\`eme d'aide \`a la d\'ecision capable de travailler \`a la granularit\'e de la ligne budg\'etaire, de signaler les d\'erives en cours d'exercice via une combinaison de score~$Z$ et d'\textit{Isolation Forest}, et d'\^etre exploit\'e directement par le gestionnaire sous la double forme d'un outil HTML autonome et d'un tableau de bord Power BI.
