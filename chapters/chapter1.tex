...
\begin{enumerate}
    ...
    \item \textbf{D\'eclinaison financi\`ere des projets, suivi des conventions et projets soutenus, et coordination avec les intervenants du domaine financier}~: gestion des partenariats, suivi des conventions de financement et articulation avec les bailleurs de fonds institutionnels.
...
\begin{itemize}
    ...
    Construire, calibrer et \'evaluer deux mod\`eles ML (Lasso et XGBoost) ainsi que deux benchmarks na\"\i fs (lag-4 et moyenne historique) selon un protocole de validation walk-forward \`a fen\^etre expansive~; mesurer leurs performances relatives via des m\'etriques standardis\'ees (RMSE, MAE, MAPE)~; analyser ce que ces r\'esultats r\'ev\`elent sur la structure des d\'epenses du minist\`ere de la Justice.

    \item \textbf{Concevoir et impl\'ementer un syst\`eme d'aide \`a la d\'ecision bas\'e sur l'IA~:}
    D\'evelopper un prototype op\'erationnel (tableau de bord interactif, syst\`eme d'alertes par ligne budg\'etaire, estimation du risque de non-ex\'ecution [VaR budg\'etaire]) illustrant comment les mod\`eles ML peuvent \^etre int\'egr\'es dans le dispositif de contr\'ole de gestion du minist\`ere conform\'ement au D\'ecret n\textsuperscript{o}~2-22-580~; formuler une feuille de route concr\`ete pour l'int\'egration progressive de ces outils.
\end{itemize}
