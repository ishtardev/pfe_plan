...
La première section pose le cadre conceptuel du ML et son positionnement par rapport
aux approches classiques. La deuxième section développe la théorie de la généralisation
et le compromis biais-variance. La troisième présente les quatre modèles évalués.
La quatrième traite d'un enjeu technique central~: comment s'assurer qu'un modèle
prédit vraiment l'avenir, et non qu'il «~triche~» en regardant des données futures.

%================================================================
\phantomsection
\subsection*{3.1 \quad Cadre conceptuel du Machine Learning en prévision budgétaire}
\addcontentsline{toc}{subsection}{3.1 Cadre conceptuel du Machine Learning en prévision budgétaire}
%================================================================

%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.1.1 \quad Positionnement méthodologique}
\addcontentsline{toc}{subsubsection}{3.1.1 Positionnement méthodologique}
%----------------------------------------------------------------

\paragraph{Définition et principe~:}

Le \textbf{Machine Learning} (ML) est une branche de l'intelligence artificielle
qui dote les systèmes informatiques de la capacité d'apprendre automatiquement
à partir de données, sans que chaque règle de décision soit codée explicitement
par un expert. L'idée fondamentale est la suivante~: plutôt que de définir manuellement
des règles (par exemple \textit{«~si le taux de réalisation en juin dépasse 40\,\%,
alors la ligne sera satisfaisante en fin d'année~»}), on soumet à l'algorithme un ensemble
d'exemples historiques dont on connaît l'issue, et il en déduit lui-même les
régularités statistiques sous-jacentes.
...
Tout projet ML suit les mêmes étapes, de la collecte des données au déploiement opérationnel. Le cycle ci-dessous est décrit dans le contexte de notre application~:
...
\begin{enumerate}
    ...
    Lasso et XGBoost ont été retenus (voir section~3.3).
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.1.2 \quad Apports du Machine Learning dans les finances publiques}
\addcontentsline{toc}{subsubsection}{3.1.2 Apports du Machine Learning dans les finances publiques}
%----------------------------------------------------------------
...
La capacit\'e d'ex\'ecution d'une ligne d\'epend de nombreux facteurs
contextuels~: la cat\'egorie budg\'etaire de la ligne (Investissement ou
Fonctionnement selon la nature de la d\'epense), le programme de rattachement
(Soutien et pilotage, Performance de l'administration judiciaire, Modernisation
du syst\`eme judiciaire et juridique, ou Renforcement des droits et des
libert\'es), la r\'egion de rattachement, la part de la ligne dans le budget
total, les ann\'ees pr\'ec\'edentes de lignes similaires. Les
algorithmes ML absorbent toutes ces informations simultan\'ement et apprennent
automatiquement \`a pond\'erer leur importance relative selon le type de d\'epense
et l'horizon de pr\'evision.
...
& \textbf{ML (Lasso / XGBoost)} \\
\hline
Non-lin\'earit\'es et effets de seuil & Non & Oui \\
Variables contextuelles multiples & Limit\'e & Naturellement int\'egr\'ees \\
Courtes s\'eries par ligne & Fragile & Robuste (mutualisation) \\
Interpr\'etabilit\'e & Coefficients & Coefficients (Lasso), importance (XGB) \\
\hline
\end{tabular}
\end{table}
...
%================================================================
\phantomsection
\subsection*{3.2 \quad Théorie de la généralisation en apprentissage supervisé}
\addcontentsline{toc}{subsection}{3.2 Théorie de la généralisation en apprentissage supervisé}
%================================================================

%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.2.1 \quad Problématique de la généralisation}
\addcontentsline{toc}{subsubsection}{3.2.1 Problématique de la généralisation}
%----------------------------------------------------------------
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.2.2 \quad Compromis biais--variance}
\addcontentsline{toc}{subsubsection}{3.2.2 Compromis biais--variance}
%----------------------------------------------------------------
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.2.3 \quad Surapprentissage et robustesse des modèles}
\addcontentsline{toc}{subsubsection}{3.2.3 Surapprentissage et robustesse des modèles}
%----------------------------------------------------------------
...
%================================================================
\phantomsection
\subsection*{3.3 \quad Modèles de prévision retenus}
\addcontentsline{toc}{subsection}{3.3 Modèles de prévision retenus}
%================================================================
...
Avant tout modèle sophistiqué, il est indispensable de définir des références simples~:
des prévisions que n'importe quel gestionnaire pourrait produire sans algorithme, et
que tout modèle ML doit surpasser pour justifier sa complexité supplémentaire.

\paragraph{Benchmark 1~: Moyenne historique.}

La prévision la plus simple consiste à regarder comment une ligne s'est comportée
par le passé et à répéter ce niveau~: si une ligne a réalisé en moyenne 82\,\% de
son budget lors des cinq dernières années, on prédit 82\,\% pour l'année en cours.
...
Une variante encore plus simple~: prévoir pour cette année exactement ce qui s'est
passé l'an dernier.
...
Ce benchmark est souvent difficile à battre, car il capte implicitement les tendances
récentes et l'inertie institutionnelle. Un modèle ML qui ne fait pas mieux qu'un
simple «~copier-coller de l'an dernier~» n'apporte aucune valeur ajoutée.
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.3.2 \quad Régression régularisée (Lasso)}
\addcontentsline{toc}{subsubsection}{3.3.2 Régression régularisée (Lasso)}
%----------------------------------------------------------------
...
aberrants à des variables peu informatives, ce qui nuit à la généralistion.

Le \textbf{Lasso} (Least Absolute Shrinkage and Selection Operator) ajoute une
contrainte~: les coefficients trop grands sont pénalisés. L'effet est double~:
les variables peu utiles se voient attribuer un coefficient exactement égal à zéro
(elles sont éliminées), et les variables retenues ont des coefficients plus stables.

$$\min_{\beta_1,\ldots,\beta_p} \;\; \underbrace{\frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y}_i)^2}_{\text{erreur de pr\'ediction}} \;+\; \lambda \underbrace{\sum_{j=1}^{p}|\beta_j|}_{\text{p\'enalit\'e sur les coefficients}}$$

\noindent\small\textit{Les $\beta_j$ sont les coefficients du mod\`ele. $\lambda$ est choisi par validation crois\'ee.}\normalsize

Le paramètre $\lambda$ contrôle l'intensité de cette contrainte~: plus $\lambda$ est
grand, plus le modèle est parcimonieux. Sa valeur est choisie automatiquement par
validation croisée (voir section~3.4).

\paragraph{Pourquoi Lasso pour les dépenses de matériel~?}
...
ce contexte, l'hypothèse de linéarité est raisonnable, et la parcimonie du Lasso
limite le surapprentissage. De plus, ses résultats sont directement interprétables~:
un gestionnaire peut comprendre qu'«~une hausse d'un point de pourcentage du taux
au deuxième trimestre se traduit par une hausse de $\beta_{T2}$ points du taux
annuel final~».
...
Imaginez un groupe d'experts qui travaillent en équipe pour faire une prévision.
Le premier expert fait une estimation rapide. Le second examine les cas où le premier
s'est trompé et tente de corriger ces erreurs. Le troisième corrige les erreurs
restantes du duo précédent, et ainsi de suite. À la fin, toutes les corrections
s'additionnent pour produire une prévision bien meilleure que celle du premier expert
seul. C'est exactement le principe du \textbf{gradient boosting}.
...
Les dépenses d'investissement présentent des \textbf{non-linéarités fortes}~: une
ligne à 5\,\% au deuxième trimestre avec un programme d'investissement infrastructure
peut tout à fait atteindre 80\,\% en fin d'année grâce à des décaissements massifs
en fin d'exercice, ce qu'un modèle linéaire ne peut pas prédire. XGBoost est
particulièrement adapté à ces situations car~:
...
\begin{enumerate}
    ...
    mécaniquement, tandis que XGBoost peut s'ajuster prudemment.
...
%================================================================
\phantomsection
\subsection*{3.4 \quad Validation en séries temporelles et fuite de données}
\addcontentsline{toc}{subsection}{3.4 Validation en séries temporelles et fuite de données}
%================================================================
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.4.1 \quad Spécificités des données temporelles}
\addcontentsline{toc}{subsubsection}{3.4.1 Spécificités des données temporelles}
%----------------------------------------------------------------

\paragraph{La validation croisée standard~: efficace mais inadaptée ici.}

La méthode classique pour évaluer un modèle ML est la \textbf{validation croisée
$k$-fold}~: on divise les données en $k$ groupes~; tour à tour, chaque groupe sert
de test pendant que le modèle s'entraîne sur les autres. La performance finale est
la moyenne sur tous les groupes.
...
Cette méthode est valide quand les observations sont indépendantes. Mais pour des
données temporelles, elle commet une erreur grave~: en mélangeant aléatoirement les
années, elle permet au modèle de \textbf{s'entraîner sur le futur pour prédire
le passé}. Un modèle qui a «~vu~» les données de 2024 pour prédire 2022 ne fait
pas vraiment de la prévision : il triche.
...
expansive}~: on simule exactement ce que vivra le gestionnaire en production.
...
La fenêtre d'entraînement \textit{s'élargit} à chaque fold~: plus on avance dans
le temps, plus le modèle dispose d'historique. La performance finale est la moyenne
des erreurs sur les quatre années de test.
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.4.3 \quad Risque de data leakage et implications}
\addcontentsline{toc}{subsubsection}{3.4.3 Risque de data leakage et implications}
%----------------------------------------------------------------
...
%----------------------------------------------------------------
\phantomsection
\subsubsection*{3.4.4 \quad Conséquences sur l'évaluation des modèles}
\addcontentsline{toc}{subsubsection}{3.4.4 Conséquences sur l'évaluation des modèles}
%----------------------------------------------------------------
...
Lasso et XGBoost ont été retenus pour leurs performances et leur adéquation au
contexte opérationnel~; Random Forest a été évalué mais écarté. Enfin, la validation
walk-forward (section~3.4) a été présentée comme le seul protocole qui simule
fidèlement les conditions de déploiement réel, en éliminant par construction tout
risque de fuite temporelle.
